...
int main()
{
	...
	RenderWindow window(VideoMode(700, 740), "The Game!");
		...
		if (Lose)
		{
			...
			for (int i = 0; i < high; i++)
			{
				for (int j = 0; j < width; j++)
				{
					...
					s_.setPosition(10 + j * 36, 10 + i * 36);
					window.draw(s_);
				}
			}
		}
		else
		{
			...
			if (timer > delay)
			{
				...
				if (!check(a))
				{
					...
					////Lose////
					for (int i = 0; i < 4; i++) if (field[a[i].y][a[i].x] != 0)	Lose = true;

					if (Lose) for (int i = 0; i < 4; i++) field[a[i].y][a[i].x] = colorNum;
			...
			if (remove_counter != 0)
			{
				...
				{
					Level_counter++;
					if (Level_counter % 10 == 0 && Level < 9) Level++;
				}
				remove_counter = 0;
			}
			...
			for (int i = 0; i < 4; i++)
			{
				s_.setPosition(10 + shadow[i].x * 36, 10 + shadow[i].y * 36);
				window.draw(s_);
			}
			...
			for (int i = 0; i < 4; i++)
			{
				...
				s.setPosition(10 + a[i].x * 36, 10 + a[i].y * 36);
				window.draw(s);
			}
